<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if(!$organization->canAcceptOnlinePayments())
                ...
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                            Your current subscription plan does not include online payment features. 
                            <a href="{{ route('subscription.index') }}" class="underline font-medium">Upgrade your plan</a> to accept online payments via eSewa, Khalti, and Stripe.
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                            ...
                            <div class="flex items-center space-x-2">
                                <form method="POST" action="{{ route('organization.payment-gateways.test', [$organization, $gateway]) }}">
                                    @csrf
                                    <button type="submit" class="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400">
                                        Test
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('organization.payment-gateways.toggle', [$organization, $gateway]) }}">
                                    @csrf
                                    <button type="submit" class="text-sm text-gray-600 hover:text-gray-800 dark:text-gray-400">
                                        {{ $gateway->is_active ? 'Disable' : 'Enable' }}
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('organization.payment-gateways.destroy', [$organization, $gateway]) }}" onsubmit="return confirm('Are you sure?')">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('organization.payment-gateways.store', $organization) }}" class="space-y-6" x-data="{ gateway: 'esewa' }">
                        @csrf

                        <!-- Gateway Selection -->
                        <div>
                            <x-input-label value="Select Payment Method" />
                            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mt-2">
                                ...
                                <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all"
                                       :class="gateway === 'bank_transfer' ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600'">
                                    <input type="radio" name="gateway_name" value="bank_transfer" x-model="gateway" class="sr-only">
                                    <div class="text-center w-full">
                                ...
                                <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all"
                                       :class="gateway === 'cash' ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600'">
                                    <input type="radio" name="gateway_name" value="cash" x-model="gateway" class="sr-only">
                                    <div class="text-center w-full">
                                        <div class="font-semibold text-gray-900 dark:text-white">Cash</div>
                                        <div class="text-xs text-gray-500">Offline</div>
                                    </div>
                                </label>
                            </div>
                        ...
                        <div x-show="gateway === 'bank_transfer'" x-transition class="space-y-4">
                            <div>
                                <x-input-label for="bank_name" value="Bank Name" />
                                <x-text-input id="bank_name" name="bank_name" type="text" class="mt-1 block w-full" placeholder="e.g., Nepal Bank Limited" />
                            </div>
                            <div>
                                <x-input-label for="account_holder" value="Account Holder Name" />
                                <x-text-input id="account_holder" name="account_holder" type="text" class="mt-1 block w-full" placeholder="e.g., Your Business Name" />
                            </div>
                            <div>
                                <x-input-label for="account_number" value="Account Number" />
                                <x-text-input id="account_number" name="account_number" type="text" class="mt-1 block w-full" placeholder="e.g., 0123456789" />
                            </div>
                            <div>
                                <x-input-label for="branch" value="Branch (Optional)" />
                                <x-text-input id="branch" name="branch" type="text" class="mt-1 block w-full" placeholder="e.g., Kathmandu Branch" />
                            </div>
                            <div>
                                <x-input-label for="bank_instructions" value="Payment Instructions (Optional)" />
                                <textarea id="bank_instructions" name="bank_instructions" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm" placeholder="e.g., Please include your booking reference in the transfer description"></textarea>
                            </div>
                        </div>

                        <!-- Cash Payment Details -->
                        <div x-show="gateway === 'cash'" x-transition class="space-y-4">
                            <div>
                                <x-input-label for="cash_instructions" value="Cash Payment Instructions" />
                                <textarea id="cash_instructions" name="cash_instructions" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm" placeholder="e.g., Pay cash at the reception desk before your appointment">Pay cash at the time of appointment.</textarea>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-6">
                            ...
                            <label class="flex items-center" x-show="['esewa', 'khalti', 'stripe'].includes(gateway)">
            ...
            <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
                <h4 class="font-semibold text-blue-900 dark:text-blue-100 mb-3">Payment Method Setup Guide</h4>
                <ul class="space-y-2 text-sm text-blue-800 dark:text-blue-200">
                    ...
                    <li><strong>Bank Transfer:</strong> Provide your bank account details. Customers will transfer payment and provide proof.</li>
                    <li><strong>Cash:</strong> Accept cash payments at your location. Customize instructions for your customers.</li>
                </ul>
            </div>
            @endif
        </div>
